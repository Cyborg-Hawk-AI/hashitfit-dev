// scripts/test-assessment-flow.js
...
async function main() {
  ...
  try {
    ...
    if (workoutPlans && workoutPlans.length > 0) {
      const { data: exercisesData, error: exercisesError } = await supaAdmin
    ...
    if (nutritionPlans && nutritionPlans.length > 0) {
      const { data: mealPlansData, error: mealPlansError } = await supaAdmin
        .from('meal_plans')
        .select('*')
